    ...
    <header class=" page-header--index">
        <div class="main-container page-header__container page-header__container--index">
            <div class="page-header__logo--index">
                ...
                <p>{{ __('landing.slogan') }}</p>
            </div>
            <div class="header__account--index">
                <a href="{{ route('login') }}" class="header__account-enter">
                    <span>{{ __('landing.login') }}</span></a>
                {{ __('landing.or') }}
                <a href="{{ route('register') }}" class="header__account-registration">
                    {{ __('landing.registration') }}
                </a>
            </div>
        </div>
    </header>
    <main>
        <div class="landing-container">
            <div class="landing-top">
                <h1>{!! __('landing.header_main') !!}</h1>
                <p>{!! __('landing.text_under_header_main') !!}</p>
                <button class="button" onclick="window.location.href='{{ route('register') }}';">{{ __('landing.create_acc') }}</button>
            </div>
                ...
                <div class="landing-instruction">
                    <div class="landing-instruction-step">
                        ...
                        <div class="instruction-description">
                            <h3>{{ __('landing.public_application') }}</h3>
                            {!! __('landing.public_application_text') !!}
                        </div>
                    </div>
                    <div class="landing-instruction-step">
                        <div class="instruction-circle  circle-choice"></div>
                        <div class="instruction-description">
                            <h3>{{ __('landing.ch_performer') }}</h3>
                            {!! __('landing.ch_performer_text') !!}
                        </div>
                    </div>
                    <div class="landing-instruction-step">
                        <div class="instruction-circle  circle-discussion"></div>
                        <div class="instruction-description">
                            <h3>{{ __('landing.discus_details') }}</h3>
                            <p>{!! __('landing.discus_details_text') !!}</p>
                        </div>
                    </div>
                    <div class="landing-instruction-step">
                        <div class="instruction-circle circle-payment"></div>
                        <div class="instruction-description">
                            <h3>{{ __('landing.payment_work') }}</h3>
                            <p>{{ __('landing.payment_work_text') }}</p>
                        </div>
                    </div>
                </div>
                <div class="landing-notice">
                    <div class="landing-notice-card card-executor">
                        <h3>{{ __('landing.performers') }}</h3>
                        <ul class="notice-card-list">
                            {!! __('landing.performers_list') !!}
                        </ul>
                    </div>
                    <div class="landing-notice-card card-customer">
                        <h3>{{ __('landing.for_customers') }}</h3>
                        <ul class="notice-card-list">
                            {!! __('landing.for_customers_list') !!}
                        </ul>
                    </div>
                </div>
            </div>
        ...
        <div class="main-container page-footer__container">
            <div class="page-footer__info">
                <p class="page-footer__info-copyright">
                    © {{ date('Y') }}, {{ __('landing.copyright') }}
                </p>
                <p class="page-footer__info-use">
                    {{ __('landing.foot_info') }}
                </p>
            </div>
            <div class="page-footer__links">
                <ul class="links__list">
                    <li class="links__item">
                        <a href="{{ route('tasks.my_tasks') }}">{{ __('landing.foot_my_tasks') }}</a>
                    </li>
                    <li class="links__item">
                        <a href="{{ route('profile.edit') }}">{{ __('landing.foot_my_profile') }}</a>
                    </li>
                    <li class="links__item">
                        <a href="{{ route('register') }}">{{ __('landing.foot_registration') }}</a>
                    </li>
                    @if(!is_null($user) && $user->isCustomer())
                        <li class="links__item">
                            <a href="{{ route('tasks.create') }}">{{ __('landing.foot_create_task') }}</a>
                        </li>
                    @endif
                </ul>
            </div>
